...
| 絵文字対応版 | [Noto Color Emoji](https://github.com/googlefonts/noto-emoji) を追加合成したバリエーション。絵文字の文字幅は全角文字に合わせる。既存フォントにも含まれる数字や一部記号などは通常文字の表示を優先し、[絵文字優先リスト](doc/emoji-override.txt) に含まれる記号は絵文字側を優先する。 | `UDEVGothic*EM*-*.ttf`<br>※ファイル名に `EM` が含まれたもの |
