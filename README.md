...
## UDEV Gothic RF (right-four)

オリジナルの [UDEV Gothic](https://github.com/yuru7/udev-gothic) に対して `4` のフォント フェイスを変更したものです。  
水平線を右に突き出すようにしています。

[Noto Color Emoji](https://github.com/googlefonts/noto-emoji) を追加合成し、文字幅を全角に合わせたバリエーションもオリジナルに追加して用意しています。
...
| 4字形変更版 | `4` の水平線を右側に突き出した字形に変更したバリエーション。 | `UDEVGothic*RF*-*.ttf`<br>※ファイル名に `RF` が含まれたもの |
| 絵文字対応版 | [Noto Color Emoji](https://github.com/googlefonts/noto-emoji) を追加合成したバリエーション。絵文字の文字幅は全角文字に合わせる。既存フォントにも含まれる数字や一部記号などは通常文字の表示を優先し、[絵文字優先リスト](doc/emoji-override.txt) に含まれる記号は絵文字側を優先する。Nerd Fonts 対応版では、TrueType のグリフ最大制限 (65535) に収まるよう、一部の合成シーケンス文字を除外している。 | `UDEVGothic*EM*-*.ttf`<br>※ファイル名に `EM` が含まれたもの |
...
- `--emoji`: Noto Color Emoji を追加合成する
...
詳細は [LICENSE](https://raw.githubusercontent.com/Hondarer/udev-gothic-rf/main/LICENSE) を参照
